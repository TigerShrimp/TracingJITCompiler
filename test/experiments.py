...
def run_java_experiment(program, runs):
    ...
    with open(os.devnull, 'wb', 0) as DEV_NULL:
        for i in range(runs):
            ...
            p = Popen(["java", "-Xlog:class+init=info:file=trace.log:timenanos",
                ...
                for line in java_init_reversed:
                    if program in line:
                        init_time = float(line.split("[")[1].split("]")[
                            0].replace("ns", ""))
                        elapsed_time = (exit_time - init_time) / 1000000
                        break
            os.remove('trace.log')
...
tests = ["TwoHotSideExits", "EvenMoreLoops", "SingleLoop",
         ...
         "FloatFibonacci", "DoubleFibonacci", "ManyVariables", "ManyVariablesMulTrace"]
...
for test in tests:
    (inter, trace, java, traces, bytesWritten) = run_experiments(test)
    with open("data.csv", 'a') as res:
        res.write("{},{},{},{},{},{}\n".format(
            test, inter, trace, java, traces, bytesWritten))
